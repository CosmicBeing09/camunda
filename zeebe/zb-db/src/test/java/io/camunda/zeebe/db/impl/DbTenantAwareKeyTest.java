...
public class DbTenantAwareKeyTest {
  ...
  @BeforeEach
  void beforeEach() {
    TENANT_KEY.recordStringContent(TENANT_ID);
  }

  @AfterEach
  void afterEach() {
    TENANT_KEY.recordStringContent("");
  }
  ...
  void shouldWriteDbStringPrefixed() {
    ...
    final var value = "foo";
    wrappedKey.recordStringContent(value);
  ...
  void shouldWriteDbCompositeKeyPrefixed() {
    ...
    stringKey.recordStringContent(stringValue);
    longKey.recordValue(longValue);
    final var tenantAwareKey = new DbTenantAwareKey<>(TENANT_KEY, wrappedKey, PlacementType.PREFIX);
