...
public class ObjectValue extends BaseValue {
  private final List<BaseProperty<? extends BaseValue>> staticProperties;
  ...
  public ObjectValue(final int expectedDeclaredProperties) {
    staticProperties = new ArrayList<>(expectedDeclaredProperties);
  }
  ...
  public void reset() {
    for (int i = 0; i < staticProperties.size(); ++i) {
      final BaseProperty<? extends BaseValue> prop = staticProperties.get(i);
      prop.reset();
    }
  ...
  public void writeJSON(final StringBuilder builder) {
    ...
    writeJson(builder, staticProperties);
  ...
  public void write(final MsgPackWriter writer) {
    final int size = staticProperties.size() + dynamicProperties.size();

    writer.writeMapHeader(size);
    write(writer, staticProperties);
  ...
  public void read(final MsgPackReader reader) {
    ...
    for (int i = 0; i < mapSize; ++i) {
      ...
      for (int k = 0; k < staticProperties.size(); ++k) {
        final BaseProperty<?> declaredProperty = staticProperties.get(k);
    ...
    for (int p = 0; p < staticProperties.size(); p++) {
      final BaseProperty<?> prop = staticProperties.get(p);
      if (!prop.hasValue()) {
  ...
  public int getEncodedLength() {
    ...
    length += getEncodedLength(staticProperties);
  ...
  @Override
  public int hashCode() {
    return Objects.hash(staticProperties, dynamicProperties, recycledProperties);
  }
  ...
  public boolean equals(final Object o) {
    ...
    return Objects.equals(staticProperties, that.staticProperties)
  ...
  public boolean isEmpty() {
    return staticProperties.isEmpty() && dynamicProperties.isEmpty();
  }
}
