...
public class DbUserTaskState implements MutableUserTaskState {
  ...
  private final UserTaskCancellation userTaskTransitionTriggerRequestMetadata =
      new UserTaskCancellation();
  private final ColumnFamily<DbLong, UserTaskCancellation>
  ...
  @Override
  public void storeTriggerData(
      final long key, final UserTaskCancellation recordRequestMetadata) {
    userTaskKey.wrapLong(key);
  ...
  @Override
  public Optional<UserTaskCancellation> findRecordRequestMetadata(
      final long key) {
    userTaskKey.wrapLong(key);
