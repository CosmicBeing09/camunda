...
public final class VariableDocumentUpdateProcessor
  ...
  private final KeyGenerator variableDocumentKeyGenerator;
  ...
  public VariableDocumentUpdateProcessor(
    ...
    variableDocumentKeyGenerator = keyGenerator;
  ...
  public void processRecord(final TypedRecord<VariableDocumentRecord> record) {
    ...
    final ElementInstance elementInstance = elementInstanceState.getInstance(value.getScopeKey());
    ...
    if (isCamundaUserTask(elementInstance)) {
      ...
      final long variableDocKey = variableDocumentKeyGenerator.nextKey();
    ...
    final long variableDocumentKey = variableDocumentKeyGenerator.nextKey();

    writers.state().appendFollowUpEvent(variableDocumentKey, VariableDocumentIntent.UPDATED, value);
    writers.response().writeEventOnCommand(variableDocumentKey, VariableDocumentIntent.UPDATED, value, record);
  }
