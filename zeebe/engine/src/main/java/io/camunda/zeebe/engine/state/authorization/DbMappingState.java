...
public class DbMappingState implements MutableMappingState {
  ...
  public void create(final MappingRecord mappingRecord) {
    ...
    this.mappingId.setValueFromString(mappingId);
    this.claimName.setValueFromString(claimName);
    claimValue.setValueFromString(value);
  ...
  public void update(final MappingRecord mappingRecord) {
    mappingId.setValueFromString(mappingRecord.getMappingId());
    get(mappingRecord.getMappingId())
        .ifPresentOrElse(
            persistedMapping -> {
              // remove old record from mapping by claim
              claimName.setValueFromString(persistedMapping.getClaimName());
              claimValue.setValueFromString(persistedMapping.getClaimValue());
  ...
  public void delete(final String id) {
    get(id)
        .ifPresentOrElse(
            persistedMapping -> {
              mappingId.setValueFromString(persistedMapping.getMappingId());
              claimName.setValueFromString(persistedMapping.getClaimName());
              claimValue.setValueFromString(persistedMapping.getClaimValue());
              mappingColumnFamily.deleteExisting(claim);
  ...
  public Optional<PersistedMapping> get(final String id) {
    mappingId.setValueFromString(id);
  ...
  public Optional<PersistedMapping> get(final String claimName, final String claimValue) {
    this.claimName.setValueFromString(claimName);
    this.claimValue.setValueFromString(claimValue);
