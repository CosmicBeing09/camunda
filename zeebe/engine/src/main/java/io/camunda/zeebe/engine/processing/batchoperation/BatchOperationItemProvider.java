...
public class BatchOperationItemProvider {
  ...
  private <F extends FilterBase> Set<Item> fetchEntityItems(
    ...
    while (true) {
      ...
      searchAfter = result.lastSearchAfter();
   ...
   * @param lastSearchAfter the the marker for fetching the next page
   * @param total the total amount of found items
   */
  private record ItemPage(final List<Item> items, final Object[] lastSearchAfter, final long total) {}
  ...
  private interface ItemPageFetcher<F extends FilterBase> {
     ...
     * @param searchAfter the marker for fetching the next page
