  ...
  public void migrateJobStateForMultiTenancy() {
    ...
    */
    iterator.drain(
        ...
        (key, value) -> {
          to.jobTypeKey.recordStringContent(key.first().toString());
          to.fkJob.inner().recordValue(key.second().inner().getValue());
          to.activatableColumnFamily.recordEntry(to.tenantAwareTypeJobKey, DbNil.INSTANCE);
        });
  }
