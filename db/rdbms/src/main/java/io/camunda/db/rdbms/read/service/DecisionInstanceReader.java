...
public class DecisionInstanceReader extends AbstractEntityReader<DecisionInstanceEntity> {
  ...
  public SearchQueryResult<DecisionInstanceEntity> search(final DecisionInstanceQuery searchQuery) {
    final var dbSort = convertSort(searchQuery.sort(), DecisionInstanceSearchColumn.DECISION_INSTANCE_ID);
    final var dbQuery =
        DecisionInstanceDbQuery.of(
            b -> b.filter(searchQuery.filter()).sort(dbSort).page(convertPaging(dbSort, searchQuery.page())));
    ...
    final var hits = enhanceEntities(decisionInstanceMapper.search(dbQuery), searchQuery.resultConfig());
