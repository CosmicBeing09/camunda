    ...
    if (operations == null || operations.isEmpty()) {
      ...
      for (final Operation<Integer> op : operations) {
        switch (op.operator()) {
          ...
          case IN -> queries.add(intTerms(field, op.valueList()));
          case NOT_IN -> queries.add(mustNot(intTerms(field, op.valueList())));
    ...
    if (operations == null || operations.isEmpty()) {
      ...
      for (final Operation<Long> op : operations) {
        switch (op.operator()) {
          ...
          case LOWER_THAN_EQUALS ->
              rangeQueryBuilder = buildRangeQuery(rangeQueryBuilder, field, b -> b.lte(op.value()));
          case IN -> queries.add(longTerms(field, op.valueList()));
          case NOT_IN -> queries.add(mustNot(longTerms(field, op.valueList())));
          ...
          op -> {
            searchQueries.add(
                switch (op.operator()) {
                  ...
                  case IN -> stringTerms(field, op.valueList());
                  case NOT_IN -> mustNot(stringTerms(field, op.valueList()));
