...
import static io.camunda.webapps.schema.descriptors.template.FlowNodeInstanceTemplate.FLOW_NODE_NAME;
...
public class FlownodeInstanceFilterTransformer
  ...
  public SearchQuery toSearchQuery(final FlowNodeInstanceFilter filter) {
    ...
    ofNullable(stringTerms(START_DATE, filter.startDateOperations())).ifPresent(queries::add);
    ofNullable(stringTerms(END_DATE, filter.endDateOperations())).ifPresent(queries::add);
    return and(queries);
  }
