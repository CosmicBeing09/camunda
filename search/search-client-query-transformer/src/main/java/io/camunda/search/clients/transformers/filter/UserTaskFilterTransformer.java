...
public class UserTaskFilterTransformer extends IndexFilterTransformer<UserTaskFilter> {
  ...
  public SearchQuery toSearchQuery(final UserTaskFilter filter) {
    ...
    ofNullable(shouldRetrieveTaskByElementName(filter.elementNames())).ifPresent(queries::add);
  ...
  private SearchQuery shouldRetrieveTaskByElementName(final List<String> elementName) {
