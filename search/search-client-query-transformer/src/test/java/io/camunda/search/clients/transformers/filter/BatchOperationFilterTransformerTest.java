...
class BatchOperationFilterTransformerTest extends AbstractTransformerTest {
  ...
  void shouldQueryByState() {
    // given
    final var filter = FilterBuilders.batchOperation(f -> f.stateOperations("ACTIVE"));
  ...
  void shouldQueryByAllFields() {
    ...
    final var filter =
        FilterBuilders.batchOperation(
            f -> f.batchOperationIds("123").stateOperations("ACTIVE").operationTypeOperations("CREATE"));
