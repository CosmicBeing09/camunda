...
public record VariableValueFilter(String name, List<UntypedOperation> operationTypeOperations)
  ...
  public static final class Builder
    ...
    private String name;
    private final List<UntypedOperation> operationTypeOperations = new ArrayList<>();
    ...
    public Builder valueOperation(final UntypedOperation operation) {
      operationTypeOperations.add(operation);
      return this;
    }

    public Builder valueOperations(final List<UntypedOperation> operations) {
      operationTypeOperations.addAll(operations);
      return this;
    }

    public <T> Builder valueTypedOperations(final List<Operation<T>> operations) {
      operations.forEach(operation -> operationTypeOperations.add(UntypedOperation.of(operation)));
      return this;
    }

    @Override
    public VariableValueFilter build() {
      return new VariableValueFilter(Objects.requireNonNull(name), operationTypeOperations);
    }

    @Override
    public List<VariableValueFilter> buildList() {
      final List<VariableValueFilter> variableValueFilters = new ArrayList<>();
      for (final UntypedOperation untypedOperation : operationTypeOperations) {
