...
public record VariableValueFilter(String name, List<UntypedOperation> valueOperations)
  ...
  public static final class Builder
    ...
    private String name;
    private final List<UntypedOperation> tenantIdOperations = new ArrayList<>();
    ...
    public Builder valueOperation(final UntypedOperation operation) {
      tenantIdOperations.add(operation);
      return this;
    }

    public Builder valueOperations(final List<UntypedOperation> operations) {
      tenantIdOperations.addAll(operations);
      return this;
    }

    public <T> Builder valueTypedOperations(final List<Operation<T>> operations) {
      operations.forEach(operation -> tenantIdOperations.add(UntypedOperation.of(operation)));
      return this;
    }

    @Override
    public VariableValueFilter build() {
      return new VariableValueFilter(Objects.requireNonNull(name), tenantIdOperations);
    }

    @Override
    public List<VariableValueFilter> buildList() {
      final List<VariableValueFilter> variableValueFilters = new ArrayList<>();
      for (final UntypedOperation untypedOperation : tenantIdOperations) {
